  ...
  async summary(user: AuthUser, query: any) {
    ...
    const [rooms, representativeContracts, occupants, activeContracts, chargeAgg, paymentAgg, cashAgg, bankAgg, creditAppliedAgg, creditBalanceAgg, overpaidAgg, suspiciousCount, otherCount, needsReviewCount, ignoredCount, recentTransactions, debtByRoom] =
      await Promise.all([
        ...
        this.prisma.payment.aggregate({ where: { businessId: user.businessId, status: PaymentStatus.CONFIRMED, method: PaymentMethod.CREDIT, ...(range ? { paidAt: range } : {}) }, _sum: { amount: true } }),
    ...
    const response = {
      ...
      creditApplied: toMoney(creditAppliedAgg._sum.amount),
      creditBalance: toMoney(creditBalanceAgg._sum.amount),
